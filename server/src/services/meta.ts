...
/* Instagram analytics via the Meta Graph API.
 *
 * Flow: a team member clicks "Connect Instagram" for a merchant → Facebook
 * OAuth dialog → callback exchanges the code for a long-lived user token →
 * we find the Page the merchant's IG business account is linked to and store
 * the Page token (Page tokens minted from a long-lived user token don't
 * expire). Syncs then pull profile counts, daily insights, media performance
 * and audience-online times into the social_* tables.
 *
 * Inert unless META_APP_ID/META_APP_SECRET are set. */
...
async function insightSeries(igUserId: string, token: string, metric: string, sinceDays: number): Promise<Map<string, number>> {
...
export async function syncAccount(accountId: string): Promise<void> {
  ...
  const [reach, impressions, profileViews, websiteClicks] = await Promise.all([
    insightSeries(ig, token, 'reach', 30),
    insightSeries(ig, token, 'impressions', 30),
    insightSeries(ig, token, 'profile_views', 30),
    insightSeries(ig, token, 'website_clicks', 30),
  ])
